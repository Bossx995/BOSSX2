...
async function handleMediaShortcut(sock, msg, shortcut) {
  ...
  if (isViewOnceMessage(quoted)) {
    await sendTextSafe(sock, jid, "🔒 View Once media download/forward করে গোপনে সংরক্ষণ করার feature দেওয়া হয়নি।");
    return true;
  }
...
async function sendMenuPhotoReply(sock, jid, caption, options = {}) {
  // Menu uses its own dedicated photo so changing the menu image does not
  // change the owner/group thumbnail used elsewhere.
  ...
  if (fs.existsSync(photoPath)) {
    try {
      ...
      logger.warn({ err: e, jid }, "menu photo send failed; falling back to text");
    }
  }
  if (!result) result = await sendTextSafe(sock, jid, caption, options);
  return result;
}
...
async function runYtDlpAudio(url) {
  ...
  if (!command) {
    throw new Error("yt-dlp পাওয়া যায়নি। Termux-এ: pkg install python && pip install -U yt-dlp");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    proc.on("error", err => fail(
      err.code === "ENOENT"
        ? new Error("yt-dlp পাওয়া যায়নি। Termux-এ: pkg install python && pip install -U yt-dlp")
        : err
    ));
...
${kickOn ? "🚪 *৩টি warning পূর্ণ হওয়ায় আপনাকে group থেকে remove করা হচ্ছে।*" : `💡 *Kick ON করার জন্য:*\n${getBotPrefix()}antistatus kick on`}`, { mentions: [sender] });
  ...
  // AUTOREACTION / AUTOADMIN are owner-only controls.
  ...
  if (command === "menu" || command === "help") {
    ...
    const menuText =
`╭━━━〔 🤖 B O S S  X  B O T 〕━━━╮
┃ 👨‍💻 𝐃𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫 : Mr bikramhacker
┃ 🤖 𝐀𝐜𝐜𝐨𝐮𝐧𝐭   : ${getBotAccountName(sock)}
...
┃ 🔗 ${getBotPrefix()}link / ${getBotPrefix()}grouplink
┃ ♻️ ${getBotPrefix()}linkreset / ${getBotPrefix()}resetlink
┃ 🔓 ${getBotPrefix()}group open/close
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃ 👋 𝐖𝐄𝐋𝐂𝐎𝐌𝐄 / 𝐆𝐎𝐎𝐃𝐁𝐘𝐄
...
┃ ⚠️ ${getBotPrefix()}vv3  (privacy-safe mode)
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃ 👑 𝐎𝐖𝐍𝐄𝐑 / 𝐒𝐔𝐃𝐎
...
┃ 🌙 ${getBotPrefix()}goodnight / ${getBotPrefix()}gn
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃ 💠 𝐁𝐎𝐒𝐒 𝐗 • 𝐏𝐎𝐖𝐄𝐑𝐄𝐃 𝐁𝐘 𝐌𝐑 𝐁𝐈𝐊𝐑𝐀𝐌𝐇𝐀𝐂𝐊𝐄𝐑
╰━━━━━━━━━━━━━━━━━━━━━━━━━━━╯`

    try {
      // Menu uses the BOSS X owner photo as a large image card.
  ...
  if (command === "promote" || command === "demote") {
    if (!isGroup(jid)) {
      await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}${command} শুধু group-এ ব্যবহার করা যাবে।`);
      return;
    }
  ...
  if (command === "allpmte") {
    if (!isGroup(jid)) {
      await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}allpmte শুধু group-এ ব্যবহার করা যাবে।`);
      return;
    }
    ...
    await sendBotReply(
      sock, jid,
      success
        ? `🛡️ ALLPMTE\n${success} জন admin থেকে demote করা হয়েছে।\n🤖 BOSS X admin হিসেবে রাখা হয়েছে।`
        : "ℹ️ অন্য কোনো admin পাওয়া যায়নি।",
      { mentions }
    );
    return;
  }
  ...
  if (command === "vgroup") {
    if (!isGroup(jid)) {
      await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}vgroup শুধু group-এ ব্যবহার করা যাবে।`);
      return;
    }
    const requesterIsOwner = isOwner(sender) || isOwner(senderPn) || isSelfMessage;
    const requesterIsAdmin = await getGroupAdmin(sock, jid, sender, senderPn);
    ...
    const vgroupText =
`{@all }❤️⃟ —̳͟͞͞💗 কাকু কাকিমা❤️⃟✿♡ —̳͟͞͞💗 জেঠু জেঠিমা ♡✿💛
♡︎༺☆༻♡︎ 💛 দাদা 🧡
 ╰‿╯ ϟ 💗 বৌদি ✯꧂♡︎༺☆༻♡︎
☬『𖤍💛💗দিদি𖤍』☬×͜× 🦋⃟💗᪲᪲᪲ জামাইবাবু🦋⃟🌼✧༝ 🦋⃟🤍বোন ༝✧♔〘 ✧༝ 🦋⃟🌺বোনাই ༝✧〙♔
(っ’-‘)╮ =͟͟͞͞♡ ✧༝ 🦋⃟💐বোন ༝✧সব ভাই ব্রাদার =͟͟͞͞♡
┈━═☆❣️ সবাইকে❣️☆═━┈
(๑′ᴗ‵๑) ✧༝ 🦋⃟ ༝✧জানানো যাইতেছে যে♡💕⃝🕊️ আজ ${time} ঘটিকায় আমাদের গ্রুপে একটা অনুষ্ঠানের❤️⃟ Group video❤️⃟ ব্যবস্থা করা হয়েছে।
...
যদি যোগদান নাও করতে পারেন তাহলে ছবিগুলো দেখে আপনার মতামত মানে ✿♡ 𝓡𝓮𝓪𝓬𝓽 ♡✿গুলো দিয়ে দেবেন🦋⃟💗᪲᪲᪲✿ִֶָ🪽་༘✮⋆˙`;
  ...
  // .vv3 does not bypass WhatsApp View-Once privacy. Regular videos can still
  // be handled by the existing media commands.
  if (command === "vv3") {
    await sendBotReply(
      sock, jid,
      "⚠️ VV3: WhatsApp View-Once ভিডিওকে bypass করে full copy/forward করার feature দেওয়া হয়নি।"
    );
    return;
  }
  ...
  if (command === "lft") {
    if (!isGroup(jid)) {
      await sendBotReply(sock, jid, `❌ ${getBotPrefix()}lft শুধু group-এ ব্যবহার করা যাবে।`);
      return;
    }
    ...
    try {
      ...
      await sendBotReply(sock, jid, `❌ Group থেকে leave করা যায়নি: ${e.message || "unknown error"}`);
    }
    return;
  }
  ...
  if (command === "atag") {
    if (!isGroup(jid)) {
      await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}atag শুধু group-এ ব্যবহার করা যাবে।`);
      return;
    }
    try {
      const metadata = await getGroupMetadataSafe(sock, jid);
  ...
  if (command === "gf") {
    if (!isGroup(jid)) {
      await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}gf শুধু group-এ ব্যবহার করা যাবে।`);
      return;
    }
    try {
      const metadata = await getGroupMetadataSafe(sock, jid);
...
নিচের View channel button-এ চাপুন।`, {
      ...
      if (!args.length) {
        ...
        if (["antilink", "anti-link", "antiadmin", "antisticker"].includes(feature)) {
          if (!isGroup(jid)) {
            await sendGroupOnlyReply(sock, jid, `❌ ${feature} শুধু group-এ ব্যবহার করা যাবে।`);
      ...
      if (!isGroup(jid)) {
        await sendGroupOnlyReply(sock, jid, "❌ antistatus শুধু group-এ ব্যবহার করা যাবে।");
      } else {
...
${getBotPrefix()}antistatus kick on/off`);
      ...
      if (!isGroup(jid)) {
        await sendGroupOnlyReply(sock, jid, `❌ ${command} শুধু group-এ ব্যবহার করা যাবে।`);
      } else {
        const value = args[0]?.toLowerCase();
        if (!["on", "off"].includes(value)) {
      ...
      if (!isGroup(jid)) {
        await sendGroupOnlyReply(sock, jid, "❌ এই command শুধু group-এ ব্যবহার করা যাবে।");
...
https://chat.whatsapp.com/${code}`);
      ...
      if (!isGroup(jid)) {
        await sendGroupOnlyReply(sock, jid, "❌ setwelcome শুধু group-এ ব্যবহার করা যাবে।");
      } else {
      ...
      if (!isGroup(jid)) {
        await sendGroupOnlyReply(sock, jid, "❌ setgoodbye শুধু group-এ ব্যবহার করা যাবে।");
      } else {
        const value = cleanCustomMessage(argsRaw);
        if (!value) {
      ...
      if (!isGroup(jid)) {
        await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}hack শুধু group-এ ব্যবহার করা যাবে।`);
        return;
      }
      ...
      if (!isGroup(jid)) return void await sendGroupOnlyReply(sock, jid, `❌ ${getBotPrefix()}gcstory শুধু group-এ ব্যবহার করা যাবে।`);
      const ctx = getContextInfo(msg);
      ...
      await sendOwnerCommandCard(
        ...
        jid,
        "📖 GCSTORY",
        "✅ Group Story published successfully"
      );
    }
  } catch (e) {
